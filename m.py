...
st.write("부위를 선택하거나, 랜덤 추천 버튼을 눌러서 적절한 스트레칭 방법과 ⚠️ 주의사항을 확인해보세요!")
...
# 선택 옵션
discomfort_area = st.selectbox("어느 부위를 선택할까요?", ["선택 안 함"] + areas)

# 랜덤 추천 버튼
if st.button("🎲 랜덤 스트레칭 추천"):
    discomfort_area = random.choice(areas)
    st.success(f"랜덤 추천된 스트레칭 부위 👉 **{discomfort_area}**")
...
# 선택한 부위 출력
if discomfort_area != "선택 안 함":
    show_exercise(discomfort_area)
else:
    st.info("부위를 선택하거나 '랜덤 스트레칭 추천' 버튼을 눌러주세요.")
